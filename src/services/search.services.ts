...
import { MediaTypeQuery, PeoPleFollow, TweetType } from '~/constants/enum'

class SearchService {
  // Vì find không aggregate được nên phải dùng aggregate để tìm kiếm
  async search({
    content,
    media_type,
    people_follow,
    ...
    if (media_type) {
      if (media_type === MediaTypeQuery.Image) {
        match['medias.type'] = MediaTypeQuery.Image
      } else if (media_type === MediaTypeQuery.Video) {
        match['medias.type'] = MediaTypeQuery.Video
      }
    }
    // Tìm kiếm theo người mình đang follow
